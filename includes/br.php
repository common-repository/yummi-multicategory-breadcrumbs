<?php
/* Temp *
function ymb_array_depth($array) {
...
function ymb_get_taxonomy_hierarchy( $taxonomy, $hide_empty = 0, $parent = 0 ) {
	// only 1 taxonomy
	...
	foreach ( $terms as $term ){
		...
		$term->children = ymb_get_taxonomy_hierarchy( $taxonomy, $hide_empty, $term->term_id );
...
function ymb_get_taxonomy_hierarchy_multiple( $taxonomies, $hide_empty = 0, $parent = 0 ) {
	if ( ! is_array( $taxonomies )  ) {
		$taxonomies = array( $taxonomies );
	}
	$results = array();
	foreach( $taxonomies as $taxonomy ){
		$terms = ymb_get_taxonomy_hierarchy( $taxonomy, $hide_empty, $parent );
		if ( $terms )
...
function ymb_recursive($array, $type = 'term_id', $parent = 0, $nbsp = ''){
	foreach($array as $key => $value){
		if(is_array($value->children)){
			...
			ymb_recursive($value->children, $type, $parent, $nbsp);
		}
	}
}
$hierarchy = ymb_get_taxonomy_hierarchy( 'category' ); // $taxonomy, $hide_empty, $parent // ymb_get_taxonomy_hierarchy_multiple( array( 'category', 'post_tag' ) )
$depth = ymb_array_depth($hierarchy);
//ymb_recursive($hierarchy, 'name');
  // [term_id] => 1
...
function yummi_breadcrumbs() {
  global $wp_query, $post, $count, $author, $bro; //$wp_post_types['post'];
  $taxonomy_cats = '';
  $count = 1;
  /* === ОПЦИИ === */
  ...
  $ylink = $ylink_before . '<a' . $ylink_attr . ' href="%1$s" rel="v:url" property="v:title">%2$s</a>' . $ylink_after;
  if( !empty($post) ) $parent_id = $parent_id_2 = $post->post_parent;
  $userdata = get_userdata($author);
  $frontpage_id = get_option('page_on_front');

  $term = get_queried_object();
  ( $term->name ) ? $current_tax_label = $term->name : null;
  ( !empty($term->term_id) && get_term_link($term->term_id) ) ? $current_url = get_term_link($term->term_id) : null;
  ( !empty($term->post_type) && !$term->post_type ) ? $pt = $term->post_type : $pt = 'post';
  ( !empty($term->parent) ) ? $parent_tax_id = $term->parent : null; // get_term($term->parent)->term_id // get_term($term->parent, $pt)
...
if ( $yshow_on_home == 0 && !is_front_page() )
  echo '<div class="breadcrumbs" itemscope itemtype="http://schema.org/BreadcrumbList" xmlns:v="http://rdf.data-vocabulary.org/#"><div typeof="v:Breadcrumb">';
elseif( $yshow_on_home == 1 )
  echo '<div class="breadcrumbs" itemscope itemtype="http://schema.org/BreadcrumbList" xmlns:v="http://rdf.data-vocabulary.org/#"><div typeof="v:Breadcrumb">';

  if ( $yshow_home_link == 1 ){
    $taxonomy_cats = '<a itemprop="item" href="' . $yhome_link . '" class="hint--top" data-hint="'. esc_attr( sprintf( __( 'Go', 'yummi-multicategory-breadcrumbs' ).' %s', $text['home'] ) ) .'" title="'. esc_attr( sprintf( __( 'Go', 'yummi-multicategory-breadcrumbs' ).' %s', $text['home'] ) ) .'" rel="v:url" property="v:title"><span itemprop="name">' . $text['home'] . '</span><meta itemprop="position" content="'.$count.'"></a>'.$ydelimiter;
    $count++;
  }

  if ( is_home() || is_front_page() ) {
      if ( $yshow_on_home == 0 ) $taxonomy_cats = '';
      $ydelimiter = '';
      $yshow_current = '';
    ...
    if( !empty($term->parent) && $term->parent == 0 ) {
  	...
  	if ( $yshow_home_link == 1 ){
  		$taxonomy_cats .= '<span rel="v:child" typeof="v:Breadcrumb"><a itemprop="item" href="'. get_author_posts_url($userdata->id, $userdata->user_nicename) .'" class="hint--top" data-hint="'. esc_attr( sprintf($text['author'], $userdata->display_name) ) .'" title="' . esc_attr( sprintf($text['author'], $userdata->display_name) ) . '" rel="v:url" property="v:title"><span itemprop="name">'. sprintf($text['author'], $userdata->display_name) .'</span><meta itemprop="position" content="'.$count.'"></a></span>';
  		$count++;
  	}else{
  		$taxonomy_cats .= '<a itemprop="item" href="'. get_author_posts_url($author->ID, $author->data->user_nicename) .'" class="hint--top" data-hint="'. esc_attr( sprintf($text['author'], $author->data->display_name) ) .'" title="' . esc_attr( sprintf($text['author'], $author->data->display_name) ) . '" rel="v:url" property="v:title"><span itemprop="name">'. sprintf($text['author'], $author->data->display_name) .'</span><meta itemprop="position" content="'.$count.'"></a>';
    ...
    if ( $has_archive ){
      if ( $yshow_home_link == 1 ){
        $taxonomy_cats .= '<span rel="v:child" typeof="v:Breadcrumb"><a itemprop="item" href="'. $yhome_link . $has_archive .'" class="hint--top" data-hint="'. esc_attr( sprintf( __( 'Articles in category', 'yummi-multicategory-breadcrumbs' ).' "%s"', $label ) ) .'" title="' . esc_attr( sprintf( __( 'Articles in category', 'yummi-multicategory-breadcrumbs' ).' "%s"', $label ) ) . '" rel="v:url" property="v:title"><span itemprop="name">'. $label .'</span><meta itemprop="position" content="'.$count.'"></a></span>'.$ydelimiter;
        $count++;
      }else{
        $taxonomy_cats .= '<a itemprop="item" href="'. $yhome_link . $has_archive .'" class="hint--top" data-hint="'. esc_attr( sprintf( __( 'Articles in category', 'yummi-multicategory-breadcrumbs' ).' "%s"', $label ) ) .'" title="' . esc_attr( sprintf( __( 'Articles in category', 'yummi-multicategory-breadcrumbs' ).' "%s"', $label ) ) . '" rel="v:url" property="v:title"><span itemprop="name">'. $label .'</span><meta itemprop="position" content="'.$count.'"></a>'.$ydelimiter;
        $count++;
      }
    }
    ...
    if ( !empty( $post_terms ) && !is_wp_error( $post_terms ) ) {
      ...
      $walker = new Yummi_Walker_Category($bro);
    ...
    if ( $yshow_home_link == 1 ){
      $taxonomy_cats .= '<span rel="v:child" typeof="v:Breadcrumb"><a itemprop="item" class="hint--top parent" data-hint="'. esc_attr( sprintf( __( 'Articles for', 'yummi-multicategory-breadcrumbs' ).' "%s"', $year_name) ) .'" title="' . esc_attr( sprintf( __( 'Articles for', 'yummi-multicategory-breadcrumbs' ).' "%s"', $year_name) ) .'" href="' . $year_link . '" rel="v:url" property="v:title"><span itemprop="name">'. $year_name .'</span><meta itemprop="position" content="'.$count.'"></a></span>' . $ydelimiter;
      $count++;
    }else{
      $taxonomy_cats .= '<a itemprop="item" class="hint--top parent" data-hint="'. esc_attr( sprintf( __( 'Articles for', 'yummi-multicategory-breadcrumbs' ).' "%s"', $year_name) ) .'" title="' . esc_attr( sprintf( __( 'Articles for', 'yummi-multicategory-breadcrumbs' ).' "%s"', $year_name) ) .'" href="' . $year_link . '" rel="v:url" property="v:title"><span itemprop="name">'. $year_name .'</span><meta itemprop="position" content="'.$count.'"></a>' . $ydelimiter;
      $count++;
    }

    $taxonomy_cats .= '<span rel="v:child" typeof="v:Breadcrumb"><a itemprop="item" class="hint--top parent" data-hint="'. esc_attr( sprintf( __( 'Articles for', 'yummi-multicategory-breadcrumbs' ).' "%s %s"', $month_name, $year_name) ) .'" title="' . esc_attr( sprintf( __( 'Articles for', 'yummi-multicategory-breadcrumbs' ).' "%s"', $month_name) ) .'" href="' . $month_link . '" rel="v:url" property="v:title"><span itemprop="name">'. $month_name .'</span><meta itemprop="position" content="'.$count.'"></a></span>' . $ydelimiter;
    $count++;
    $taxonomy_cats .= '<span rel="v:child" typeof="v:Breadcrumb"><a itemprop="item" class="hint--top parent" data-hint="'. esc_attr( sprintf( __( 'Articles for', 'yummi-multicategory-breadcrumbs' ).' "%s %s %s"', $day_name,  $month_name, $year_name) ) .'" title="' . esc_attr( sprintf( __( 'Articles for', 'yummi-multicategory-breadcrumbs' ).' "%s %s %s"', $day_name,  $month_name, $year_name) ) .'" href="' . $day_link . '" rel="v:url" property="v:title"><span itemprop="name">'. $day_name .'</span><meta itemprop="position" content="'.$count.'"></a></span>' . $ydelimiter;
...
class Yummi_Walker_Category extends Walker_Category {
  /*private $_options;
  ...
  function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
    ...
    if( $category->parent == 0 || $bro['ysubcategory'] == 'first' ){ //3 count($termchildren) > 0
      $ylink = ' <span rel="v:child" typeof="v:Breadcrumb"><a itemprop="item" class="hint--top parent" data-hint="'. esc_attr( sprintf( __( 'Articles in category', 'yummi-multicategory-breadcrumbs' ).' "%s"', $cat_name) ) .'" title="' . esc_attr( sprintf( __( 'Articles in category', 'yummi-multicategory-breadcrumbs' ).' "%s"', $cat_name) ) .'" href="'.esc_url( get_term_link($category) ).'" rel="v:url" property="v:title"><span itemprop="name">'.$cat_name.'</span><meta itemprop="position" content="'.$count.'"></a></span>';
      $count++;
    }else{
